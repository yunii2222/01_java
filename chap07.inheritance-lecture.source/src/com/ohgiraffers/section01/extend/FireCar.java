...
public class FireCar extends Car {
    ...
    * 오버라이딩 하는 메소드는 기본적으로 부모 메소드 선언 내용을 그대로 작성해야 한다. */
    ...
    public void soundHorn() {
        if (isRunning()){
            /* runningStatus 는 private 필드이므로 불가능하며,
            * 부모의isRunning() 메소드를 통해서만 주행 중인 상태를 확인할 수 있으므로 메소드의 접근 제한자를 protected로 변경한다.
            * Car클래스의 runnungStatus를 protected로 변경하여 직접 접근할 수도 있지만 캡슐화의 원칙에 위배되므로 권장 되지 않는다.
            * this.isRunning() or super.isRunning() 둘 다 사용 가능하며 컴파일러는 기본적으로 this.를 추가한다
            * (상황에 따라서는 super. 를 명시적으로 써야 하는 상황도 있다.) */
            System.out.println(" 빠아아아아앙~!빠아아아아앙아아야아야앙!!! ");
        }else {
            System.out.println(" 비키세요!! ");
        }
    }
    
    /* 상속은 자식 클래스가 부모 클래스를 확장하는 것이다. 따라서 FireCar만의  */
    public void sprayWater() {
        System.out.println(" 물을 뿌릴거에요 ");
    }
