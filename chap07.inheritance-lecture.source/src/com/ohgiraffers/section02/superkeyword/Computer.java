...
public class Computer extends Product{
    ...
    public Computer(String code, String brand, String name, int price, Date manufaturingDate, String cpu, int hdd, int ram, String operationSystem) {
        ...
        /* 위에 작성 된 생성자를 this()로 호출 한다는 것은 super()를 두 번 호출하는 것곽 같기 때문에 허용되지 않는다.
        * (부모 인스턴스를 두 개 생성할 수 없기 때문에 부모 생성자는 한 번만 호출이 가능하다.) */
//        this.(cpu, hdd, ram, operationSystem);
        this.cpu = cpu;
        this.hdd = hdd;
        this.ram = ram;
        this.operationSystem = operationSystem;
        System.out.println(" Computer 클래스의 모든 필드를 초기화 하는생성자 호출 ");
    }

    /* getter, setter는 부모 필드의 메소드에 대해서 자신의 것처럼 사용 가능하므로 별도 작성이 필요없다.
...
//                + " ]";
         ...
         * 부모 클래스의 메소드를 호출해서 반환 받은 문자열이 현재 클래스의 필드 값을 추가로 덧붙여본다. */
        ...
        *  */
        ...
        return super.getInformation()
                ...
                + "cpu = " +  this.cpu
                + "hdd = " + this.hdd
                + "manufaturingDate = " + this.operationSystem
                + " ]";
    }
